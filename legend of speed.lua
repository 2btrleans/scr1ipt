...
local Categories = {"TELEPORT", "PLAYER", "MOVEMENT", "RENDER", "HUD"}
...
createToggle("Auto Collect Steps", "PLAYER", function(on)
    if on then
        task.spawn(function()
            while isToggleActive("Auto Collect Steps") do
                pcall(function()
                    orbEvent:FireServer("collectOrb", "Red Orb", "City")
                    orbEvent:FireServer("collectOrb", "Blue Orb", "City")
                end)

                task.wait(0.0001) -- çok hızlı spam
            end
        end)
    end
end)
...
createToggle("Jungle City", "TELEPORT", function(on)
    if on then
        task.spawn(function()
            local player = game.Players.LocalPlayer
            ...
            -- Belirlenen koordinatlar
            local targetPosition = Vector3.new(1471.5771484375, 19.548797607421875, 12880.302734375)
            ...
            -- Toggle'u kapatma
            local frame = nil
            for _, catFrame in pairs(CategoryFrames) do
                for _, child in ipairs(catFrame:GetChildren()) do
                    if child:IsA("TextButton") and child.Text:find("Jungle City") then
                        frame = child
                        break
                    end
                end
            ...
            -- Bildirim
            Notify("Teleported to Jungle City!", 3, Color3.fromRGB(0,255,0))
        end)
    end
end)


createToggle("Snow City", "TELEPORT", function(on)
    if on then
        task.spawn(function()
            local player = game.Players.LocalPlayer
            local character = player.Character or player.CharacterAdded:Wait()
            ...
            -- Snow City koordinatları
            local targetPosition = Vector3.new(-866.3868408203125, 15.879983901977539, 2165.70654296875)
            ...
            for _, catFrame in pairs(CategoryFrames) do
                for _, child in ipairs(catFrame:GetChildren()) do
                    if child:IsA("TextButton") and child.Text:find("Snow City") then
                        frame = child
                        break
                    end
                end
            ...
            -- Bildirim
            Notify("Teleported to Snow City!", 3, Color3.fromRGB(0,255,0))
        end)
    end
end)


createToggle("Cave", "TELEPORT", function(on)
    if on then
        task.spawn(function()
            local player = game.Players.LocalPlayer
            local character = player.Character or player.CharacterAdded:Wait()
            ...
            for _, catFrame in pairs(CategoryFrames) do
                for _, child in ipairs(catFrame:GetChildren()) do
                    if child:IsA("TextButton") and child.Text:find("Cave") then
                        frame = child
                        break
                    end
                end
            ...
            -- Bildirim
            Notify("Teleported to Cave!", 3, Color3.fromRGB(0,255,0))
        end)
    end
end)

createToggle("Electro Cave", "TELEPORT", function(on)
    if on then
        task.spawn(function()
            local player = game.Players.LocalPlayer
            local character = player.Character or player.CharacterAdded:Wait()
            ...
            -- Electro Cave koordinatları
            local targetPosition = Vector3.new(3678.62548828125, 81.77999877929688, 5592.3525390625)
            ...
            for _, catFrame in pairs(CategoryFrames) do
                for _, child in ipairs(catFrame:GetChildren()) do
                    if child:IsA("TextButton") and child.Text:find("Electro Cave") then
                        frame = child
                        break
                    end
                end
            ...
            -- Bildirim
            Notify("Teleported to Electro Cave!", 3, Color3.fromRGB(0,255,0))
        end)
    end
end)
...
createToggle("Magma City", "TELEPORT", function(on)
    if on then
        task.spawn(function()
            local player = game.Players.LocalPlayer
            local character = player.Character or player.CharacterAdded:Wait()
            ...
            -- Magma City koordinatları
            local targetPosition = Vector3.new(1616.8270263671875, 14.279984474182129, 4330.65234375)
            ...
            for _, catFrame in pairs(CategoryFrames) do
                for _, child in ipairs(catFrame:GetChildren()) do
                    if child:IsA("TextButton") and child.Text:find("Magma City") then
                        frame = child
                        break
                    end
                end
            ...
            -- Bildirim
            Notify("Teleported to Magma City!", 3, Color3.fromRGB(0,255,0))
        end)
    end
end)
...
createToggle("Auto Collect Gems", "PLAYER", function(on)
    if on then
        task.spawn(function()
            while isToggleActive("Auto Collect Gems") do
                pcall(function()
                    -- City
                    orbEvent:FireServer("collectOrb", "Gem", "City")
                    -- Desert
                    orbEvent:FireServer("collectOrb", "Gem", "Desert")
                    -- Magma City
...
createToggle("Auto Rebirth", "PLAYER", function(on)
    if on then
        task.spawn(function()
            while isToggleActive("Auto Rebirth") do
                pcall(function()
                    rebirthEvent:FireServer("rebirthRequest")
                end)

                task.wait(0.1) -- biraz bekletiyoruz (0.1 sn), çok hızlı olursa lag olabilir
            end
        end)
    end
end)
...
local rEvents = ReplicatedStorage:WaitForChild("rEvents")
local openCrystalRemote = rEvents:WaitForChild("openCrystalRemote")

local lastOpen = 0
local delayTime = 0.05 -- saniye (20 kez/s)

-- Toggle
createToggle("AutoEGG", "PLAYER", function(on)
    if on then
        connection = RunService.Heartbeat:Connect(function()

            if not isToggleActive("AutoEGG") then
                return
            end

            local now = tick()

            if now - lastOpen >= delayTime then
                lastOpen = now

                local selectedCrystal = getMode("AutoEGG", "Mode")
...
-- Mode ekleme
addMode("AutoEGG", "Mode", allCrystals, 1)
...
createToggle("Auto Collect EXP", "PLAYER", function(on)
    if on then
        task.spawn(function()
            while isToggleActive("Auto Collect EXP") do
                pcall(function()
...
-- Toggle
createToggle("AutoSell", "PLAYER", function(on)
    if on then
        task.spawn(function()
            while isToggleActive("AutoSell") do
                pcall(function()
                    local rEvents = ReplicatedStorage:WaitForChild("rEvents")
                    local sellPetEvent = rEvents:WaitForChild("sellPetEvent")
                    local sellTrailEvent = rEvents:WaitForChild("sellTrailEvent")

                    -- PETS
                    local petsFolder = LocalPlayer:WaitForChild("petsFolder")
                    for _, rarity in pairs({"Basic", "Epic", "Omega", "Rare", "Unique"}) do
                        if getBoolean("AutoSell", rarity) then
                            local folder = petsFolder:FindFirstChild(rarity)
                            if folder then
                    ...
                    if trailsFolder then
                        for _, rarity in pairs({"Basic", "Advanced", "Epic", "Rare", "Unique"}) do
                            if getBoolean("AutoSell", rarity) then
                                local folder = trailsFolder:FindFirstChild(rarity)
                                if folder then
...
-- Örnek boolean ekleme
addBoolean("AutoSell", "Basic", true)
addBoolean("AutoSell", "Epic", false)
addBoolean("AutoSell", "Omega", false)
addBoolean("AutoSell", "Rare", false)
addBoolean("AutoSell", "Unique", false)

-- Trails için
addBoolean("AutoSell", "Advanced", true)
addBoolean("AutoSell", "Basic", true)
addBoolean("AutoSell", "Epic", false)
addBoolean("AutoSell", "Rare", false)
addBoolean("AutoSell", "Unique", false)
...
addSlider("Speed", "Speed",100,1000,1000)
addSlider("HighJump", "Power",50,500,200)
