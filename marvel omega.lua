...
local function addBillboard(part)
    if not part:FindFirstChild("DistanceGui") then
...
local function updateDistance(part)
    local gui = part:FindFirstChild("DistanceGui")
...
-- ✅ GÜNCELLENMİŞ TARAMA MANTIĞI
function scanChestParts()
    for _, obj in pairs(workspace:GetDescendants()) do
        -- 1. ChestController'ı bul
        if obj:IsA("LuaSourceContainer") and obj.Name == "ChestController" then
            local parent = obj.Parent
            -- 2. Aynı model içindeki Body'yi bul
            local body = parent:FindFirstChild("Body")
            
            if body then
                -- 3. Body içindeki PromptAttachment'ı bul
                local promptAttachment = body:FindFirstChild("PromptAttachment")
                if promptAttachment then
                    -- 4. PromptAttachment içindeki ProximityPrompt'u bul
...
-- ✅ ESP Toggle
createToggle("ChestESP", "RENDER", function(on)
    if on then
        scanChestParts()
    else
        for part, _ in pairs(trackedParts) do
            if part:FindFirstChild("ESP_Highlight") then part.ESP_Highlight:Destroy() end
            if part:FindFirstChild("DistanceGui") then part.DistanceGui:Destroy() end
        end
        trackedParts = {}
    end
end)
...
-- ✅ Mesafe Güncelleme
RunService.RenderStepped:Connect(function()
    if isToggleActive("ChestESP") then
...
task.spawn(function()
    while true do
        if isToggleActive("ChestESP") or isToggleActive("AutoCollectChest") then
            scanChestParts()
        end
        task.wait(5)
    end
...
local function interactPrompt(prompt)
    ...
    -- Süreyi her ihtimale karşı tekrar 0 yap
    prompt.HoldDuration = 0
...
local function teleportToPart(part)
    ...
    if hrp and part then
        -- Body'nin tam üzerine değil biraz üstüne ışınlan (takılmamak için)
...
function autoCollectLoop()
    while collectLoop do
        ...
        for bodyPart in pairs(trackedParts) do
            if not collectLoop then break end
            
            if bodyPart and bodyPart.Parent then
                -- Hiyerarşiyi takip et: Body -> PromptAttachment -> ProximityPrompt
                local att = bodyPart:FindFirstChild("PromptAttachment")
                local prompt = att and att:FindFirstChildWhichIsA("ProximityPrompt")

                if prompt and prompt.Parent then
                    foundAnything = true
                    teleportToPart(bodyPart)
                    task.wait(0.3)
                    interactPrompt(prompt)
                    task.wait(0.5) -- Toplama animasyonu/işlemi için bekleme
                end
            end
        end
        
        -- Eğer etrafta chest yoksa veya hepsi toplandıysa biraz daha uzun bekle
        task.wait(2)
    end
end

createToggle("AutoCollectChest", "RENDER", function(on)
    collectLoop = on
    if on then
        scanChestParts() -- Başlamadan önce bir kez tara
...
addSlider("Speed", "Speed",20,100,50)
