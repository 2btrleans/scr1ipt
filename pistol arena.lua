...
-- CHAMS (Highlight)
local function createChamsESP(player, character)
...
local function createESP(player, character)
	...
	elseif mode == "Chams" then
		createChamsESP(player, character)
	end
end

-- Oyuncu setup (EN ÖNEMLİ KISIM)
...
-- Oyundan çıkanları temizle
Players.PlayerRemoving:Connect(function(player)
	removeESP(player)
end)

-- Ana loop (update + güvenlik)
RunService.RenderStepped:Connect(function()
		...
		if player ~= LocalPlayer then
			...
			if char and char:FindFirstChild("Humanoid") then
				if not ESPs[player] then
					...
					-- Mode değiştiyse reset
					if (currentMode == "Box" and esp.Name ~= "ESP_BOX") or
					   (currentMode == "Chams" and esp.Name ~= "ESP_CHAMS") then
						removeESP(player)
						createESP(player, char)
					else
						-- Renk güncelle
