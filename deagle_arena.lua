...
-- Belirtilen GUI öğelerini başlangıçta silen kod
task.spawn(function()
    local Players = game:GetService("Players")
    local player = Players.LocalPlayer
    local playerGui = player:WaitForChild("PlayerGui")

    -- MainGui'nin yüklenmesini bekliyoruz
    local mainGui = playerGui:WaitForChild("MainGui", 10)

    if mainGui then
        -- Silinecek frame isimlerini bir liste olarak tutalım
        local targets = {"FreeButtons", "ExclusiveButtons"}

        for _, frameName in ipairs(targets) do
            local frame = mainGui:FindFirstChild(frameName)
            if frame then
                frame:Destroy()
                print("2btr Hub: " .. frameName .. " başarıyla silindi.")
            end
        end
    end
end)
...
createToggle("Hitbox", "COMBAT", function(on)
    -- Tarama aşağıdaki döngü tarafından yapılacak
end)
...
local function resetHitbox(root)
    if root and originalSizes[root] then
        local original = originalSizes[root]
...
-- ESP gibi 1 saniyede bir tarama
task.spawn(function()
    while task.wait(1) do

        if isToggleActive("Hitbox") then

            local size = getSlider("Hitbox", "Size") or 10
            ...
            -- Hitbox kapatıldıysa eski boyutlara döndür
...
-- Oyuncu/bot ölür veya workspace'ten silinirse eski kayıtları temizle
task.spawn(function()
    while task.wait(1) do
        for root, _ in pairs(originalSizes) do
...
local ESPs = {} -- Mevcut ESP nesneleri
local ValidTargets = {} -- Tarama sonucu bulunan botlar ve oyuncular

-- ESP Temizleme
local function removeESP(model)
	if ESPs[model] then
		if ESPs[model].visible then ESPs[model].visible:Destroy() end
		if ESPs[model].hidden then ESPs[model].hidden:Destroy() end
		ESPs[model] = nil
	end
end
...
local function createESP(model)
	if not model or ESPs[model] then return end
	if model == LocalPlayer.Character then return end

	local visibleHighlight = Instance.new("Highlight")
	visibleHighlight.Adornee = model
	...
	visibleHighlight.Parent = model

	local hiddenHighlight = Instance.new("Highlight")
	hiddenHighlight.Adornee = model
...
-- [OPTIMIZASYON] Haritayı yavaşça tarayan fonksiyon (Saniyede 1 kez çalışır)
task.spawn(function()
	while task.wait(1) do -- 1 saniyelik bekleme performansı korur
		if isToggleActive("ESP") then
			local newTargets = {}
			-- Sadece workspace'in ana dallarına bakmak genellikle yeterlidir
			-- Eğer botlar çok derin klasörlerdeyse GetDescendants kullanılabilir ama wait(1) olduğu için kasmaz
			for _, obj in ipairs(workspace:GetDescendants()) do 
...
-- [PERFORMANS] RenderStepped sadece görsel güncellemeleri yapar, tarama yapmaz!
RunService.RenderStepped:Connect(function()
	if not isToggleActive("ESP") then
		for model, _ in pairs(ESPs) do removeESP(model) end
		return
	end

	local vCol = getColor("ESP","Visible Color") or Color3.fromRGB(0,255,0)
	local hCol = getColor("ESP","Hidden Color") or Color3.fromRGB(255,0,0)

	-- Mevcut hedeflere ESP ekle veya güncelle
	for _, model in ipairs(ValidTargets) do
		if model and model.Parent then
			if not ESPs[model] then
				createESP(model)
			else
				ESPs[model].visible.FillColor = vCol
				ESPs[model].hidden.FillColor = hCol
			end
		end
	end

	-- Listeden çıkan veya silinen modelleri temizle
	for model, _ in pairs(ESPs) do
		local stillValid = false
		for _, target in ipairs(ValidTargets) do
			if target == model then stillValid = true break end
		end
		
		if not stillValid or not model.Parent then
			removeESP(model)
		end
	end
end)
