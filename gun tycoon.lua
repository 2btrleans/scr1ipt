...
createToggle("Laser Destroyer", "PLAYER", function(on)
    if on then
        getgenv().LaserDestroyerRunning = true
        task.spawn(function()
            while getgenv().LaserDestroyerRunning do
                for _, obj in pairs(workspace:GetDescendants()) do
                    if obj.Name == "OwnerOnlyDoor" then
                        obj:Destroy()
                    end
                end
                task.wait(30) -- 30 saniye bekle
            end
        end)
    else
        getgenv().LaserDestroyerRunning = false
    end
end)


createToggle("NoClip", "PLAYER", function(on)
    if on then
        getgenv().NoClipRunning = true
        task.spawn(function()
            while getgenv().NoClipRunning do
                local player = game.Players.LocalPlayer
        ...
        getgenv().NoClipRunning = false
        -- Kapatıldığında karakteri normale döndür
        local player = game.Players.LocalPlayer
        if player and player.Character then
            for _, part in pairs(player.Character:GetDescendants()) do
                if part:IsA("BasePart") then
...
addBoolean("Auto Buy", "Prio Dropper", true)

createToggle("Auto Buy", "PLAYER", function(on)
    if not on then
        _G.AutoBuyRunning = false
        print("[TycoonModule] ⛔ Auto Buy kapatıldı.")
        return
    end

    print("[TycoonModule] ✅ Auto Buy aktif edildi.")
    ...
    local function parseNumber(str)
        ...
        if str:find("k") then num = num * 1_000
        elseif str:find("m") then num = num * 1_000_000
        elseif str:find("b") then num = num * 1_000_000_000 end
        return num
    end
    ...
    local function teleportTo(char, position)
        ...
        elseif char.PrimaryPart then
            char:SetPrimaryPartCFrame(CFrame.new(position + Vector3.new(0, 3, 0)))
        else
    ...
    task.spawn(function()
        while _G.AutoBuyRunning do
            ...
            local prioDropper = getBoolean("Auto Buy", "Prio Dropper")

            for _, tycoon in ipairs(tycoonsFolder:GetDescendants()) do
                if tycoon:IsA("ObjectValue") and tycoon.Name == "Owner" and tycoon.Value == player then
                    local buttons = tycoon.Parent:FindFirstChild("Buttons")
                    if not buttons then continue end

                    local priorityButtons = {}
                    local normalButtons = {}

                    for _, buttonModel in ipairs(buttons:GetChildren()) do
                        local head = buttonModel:FindFirstChild("Head")
                        local gamepassVal = buttonModel:FindFirstChild("Gamepass")
                        local rebirthsVal = buttonModel:FindFirstChild("Rebirths")

                        -- 🎯 Dropper önceliği (ismi dropper içerecek, head varsa ve görünür olacak, gamepass/rebirth olmayacak)
                        if string.find(buttonModel.Name:lower(), "dropper")
                            and head and head.Transparency == 0
                            and not gamepassVal and not rebirthsVal then

                            table.insert(priorityButtons, buttonModel)
                        else
                            table.insert(normalButtons, buttonModel)
                        end
                    end

local prioDropper = {} -- Öncelikli butonlar (Dropper)
local normalButtons = {} -- Diğer butonlar

for _, buttonModel in ipairs(buttons:GetChildren()) do
    local priceVal = buttonModel:FindFirstChild("Price")
    local headPart = buttonModel:FindFirstChild("Head")
    if not (priceVal and headPart) then continue end
    ...
    local price = parseNumber(priceVal.Value)
    local cash = parseNumber(cashVal.Value)
    if not (price and cash) then continue end

    local hasGamepass = buttonModel:FindFirstChild("Gamepass") ~= nil
    local hasRebirth = buttonModel:FindFirstChild("Rebirths") ~= nil
    if hasGamepass or hasRebirth then
        continue
    end

    -- Eğer ismi Dropper içeriyorsa ve transparency 0 ise öncelikli butona ekle
    if string.find(buttonModel.Name, "Dropper") and headPart.Transparency == 0 and price <= cash then
        table.insert(prioDropper, buttonModel)
    elseif headPart.Transparency == 0 and price <= cash then
        table.insert(normalButtons, buttonModel)
    end
end

-- Öncelikli Dropper varsa onları al
if #prioDropper > 0 then
    for _, buttonModel in ipairs(prioDropper) do
        local headPart = buttonModel:FindFirstChild("Head")
        if headPart then
            print(string.format("[TycoonModule] 💥 Öncelikli Dropper alınıyor: %s", buttonModel.Name))
            teleportTo(char, headPart.Position)
            task.wait(2)
        end
    end
else
    -- Dropper yoksa diğer butonları al
    for _, buttonModel in ipairs(normalButtons) do
        local headPart = buttonModel:FindFirstChild("Head")
        if headPart then
            print(string.format("[TycoonModule] ⚙️ Normal buton alınıyor: %s", buttonModel.Name))
            teleportTo(char, headPart.Position)
            task.wait(2)
        end
    end
