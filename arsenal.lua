...
createToggle("TorsoSize", "RENDER", function(on)
	if on then
		print("TorsoSize aktif")

		RunService.Heartbeat:Connect(function()
			if not isToggleActive("TorsoSize") then return end
			...
			for _, player in pairs(Players:GetPlayers()) do
				if player ~= Players.LocalPlayer and player.Character then
					...
					if upper then upper.Size = Vector3.new(15, 15, 15) end
					if lower then lower.Size = Vector3.new(15, 15, 15) end
				end
			end
		end)

	else
		print("TorsoSize kapalı")
		...
		for _, player in pairs(Players:GetPlayers()) do
			if player.Character then
				...
				if upper then upper.Size = Vector3.new(2, 2, 1) end
				if lower then lower.Size = Vector3.new(2, 2, 1) end
			end
		end
	end
end)
...
createToggle("No Spread", "COMBAT", function(on)
	local replicated = game:GetService("ReplicatedStorage")
	local weaponsFolder = replicated:FindFirstChild("Weapons")
	if not weaponsFolder then return end

	-- Orijinal değerler kaydedilecek
	_G.OriginalSpreadValues = _G.OriginalSpreadValues or {}
	...
	for _, v in pairs(weaponsFolder:GetDescendants()) do
		if v.Name == "MaxSpread" or v.Name == "Spread" or v.Name == "SpreadControl" then
			if on then
				...
				if not _G.OriginalSpreadValues[v] then
					_G.OriginalSpreadValues[v] = v.Value
				end
				v.Value = 0
			else
				-- Eski haline döndür
				if _G.OriginalSpreadValues[v] then
					v.Value = _G.OriginalSpreadValues[v]
				else
					v.Value = 1
				end
			end
...
-- Infinite Ammo v1
createToggle("Infinite Ammo", "COMBAT", function(on)
	local replicated = game:GetService("ReplicatedStorage")
	local curse = replicated:FindFirstChild("wkspc") and replicated.wkspc:FindFirstChild("CurrentCurse")
	if curse then
		if on then
...
createToggle("Fast Reload", "COMBAT", function(on)
	local replicated = game:GetService("ReplicatedStorage")
	local weaponsFolder = replicated:FindFirstChild("Weapons")
	if not weaponsFolder then return end

	-- Orijinal değerleri kaydetmek için tablo
	_G.OriginalReloadValues = _G.OriginalReloadValues or {
		ReloadTime = {},
		EReloadTime = {}
	}
	...
	for _, weapon in pairs(weaponsFolder:GetChildren()) do
		...
		if weapon:FindFirstChild("ReloadTime") then
			if on then
				if not _G.OriginalReloadValues.ReloadTime[weapon] then
					_G.OriginalReloadValues.ReloadTime[weapon] = weapon.ReloadTime.Value
				end
				weapon.ReloadTime.Value = 0.01
			else
				if _G.OriginalReloadValues.ReloadTime[weapon] then
					weapon.ReloadTime.Value = _G.OriginalReloadValues.ReloadTime[weapon]
				else
		...
		-- EReloadTime kontrolü
		if weapon:FindFirstChild("EReloadTime") then
			if on then
				if not _G.OriginalReloadValues.EReloadTime[weapon] then
					_G.OriginalReloadValues.EReloadTime[weapon] = weapon.EReloadTime.Value
				end
				weapon.EReloadTime.Value = 0.01
			else
				if _G.OriginalReloadValues.EReloadTime[weapon] then
					weapon.EReloadTime.Value = _G.OriginalReloadValues.EReloadTime[weapon]
				else
