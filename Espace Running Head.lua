...
createToggle("Destroy Felipe", "PLAYER", function(on)
    if on then
        print("[Destroy Felipe] ✅ Aktif edildi.")

        task.spawn(function()
            while on and task.wait(1) do  -- her 1 saniyede bir kontrol
                for _, obj in ipairs(game.Workspace:GetDescendants()) do
                    if obj:IsA("Model") and string.find(obj.Name, "Felipe") then
                        print("[Destroy Felipe] 🗑️ Silindi ->", obj:GetFullName())
                        obj:Destroy()
                    end
                end
            end
        ...
        print("[Destroy Felipe] ❌ Devre dışı bırakıldı.")
    end
end)
...
createToggle("Auto Coin", "PLAYER", function(on)
    local Players = game:GetService("Players")
            ...
            while getgenv().AutoCoin do
                ...
                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("BasePart") and obj.Name == "Coin" then
                        found = true
                        fireTouch(obj)
                        print("[Auto Coin] 💰 Coin toplandı ->", obj:GetFullName())
                    end
                end

                if not found then
                    warn("[Auto Coin] ⚠️ Coin bulunamadı.")
                end
...
createToggle("Fly", "MOVEMENT", function(on)
    ...
    if on then
        ...
        task.spawn(function()
            local speed = 65  -- Hızı buradan değiştirebilirsin

            while getgenv().IsFly do
                task.wait()

                if not char or not hrp then continue end

                -- Hareket yönünü klavyeye göre hesapla
...
createToggle("Destroy KillPart", "PLAYER", function(on)
    if on then
        getgenv().DestroyKillPart = true
        print("[DestroyKillPart] ✅ Aktif — KillPart objeleri siliniyor...")

        task.spawn(function()
            while getgenv().DestroyKillPart do
                task.wait(2)

                for _, obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("BasePart") and obj.Name:lower() == "killpart" then
                        print("[DestroyKillPart] 🔥 KillPart yok edildi →", obj:GetFullName())
                        obj:Destroy()
                    end
                end
            end
        ...
        getgenv().DestroyKillPart = false
        print("[DestroyKillPart] ❌ Devre dışı.")
    end
end)
...
createToggle("NoClip", "PLAYER", function(on)
    ...
    if on then
        ...
        print("[NoClip] ✅ Aktif — Duvarlardan geçebilirsin.")
        ...
        task.spawn(function()
            while getgenv().NoClipRunning do
                ...
                task.wait() -- her frame yenile
            end
        end)

    else
        getgenv().NoClipRunning = false
        print("[NoClip] ❌ Devre dışı — Çarpışmalar normale döndü.")
...
createToggle("Next Stage", "TELEPORT", function(on)
    local player = game.Players.LocalPlayer

    if on then
        print("[Teleport] ✅ Teleporter aranıyor...")

        local folder = workspace:FindFirstChild("Folder")
        if not folder then
            print("[Teleport] ❌ Workspace içinde 'Folder' bulunamadı!")
            return
        end

        local teleporter = folder:FindFirstChild("Teleporter")
        if not teleporter then
            print("[Teleport] ❌ Folder içinde 'Teleporter' modeli bulunamadı!")
            return
        end

        -- Teleporter'ın BasePart bul
        local tpPart = nil
        ...
        if not tpPart then
            print("[Teleport] ❌ Teleporter modelinin içinde ışınlanacak BasePart bulunamadı!")
            return
        end
        ...
        if hrp then
            ...
            print("[Teleport] ✅ Teleporter bulundu ve ışınlandın →", tpPart:GetFullName())
        else
            print("[Teleport] ⚠️ HRP bulunamadı, ışınlama yapılamadı!")
        end

    else
        print("[Teleport] ⛔ Teleport kapatıldı.")
    end
end)
...
addSlider("Speed", "Speed",20,50,50)
