...
-- ==========================================
-- ENABLE JUMP + TP JUMP (MOVEMENT) MODÜLÜ
-- ==========================================

local UserInputService = game:GetService("UserInputService")

-- Ayarlar: Hem normal zıplama gücü hem de ışınlanma mesafesi
addSlider("Enable Jump", "Jump Power", 0, 150, 50)
addSlider("Enable Jump", "TP Boost", 0, 20, 1) -- Zıplarken ne kadar yukarı ışınlanacağı

createToggle("Enable Jump", "MOVEMENT", function(on)
...
-- 1. MANTIK: Zıplama Yeteneğini Zorla Açma (Normal Zıplama için)
RunService.Heartbeat:Connect(function()
    if isToggleActive("Enable Jump") then
        local char = Player.Character
        local hum = char and char:FindFirstChild("Humanoid")
        ...
        if hum then
            ...
            -- Eğer oyun zıplama gücünü sıfırlarsa, slider'daki değeri uygula
            if hum.JumpPower < 1 then
                hum.JumpPower = getSlider("Enable Jump", "Jump Power") or 50
            end
        end
    end
end)

-- 2. MANTIK: TP Jump Desteği (Işınlanma için)
UserInputService.JumpRequest:Connect(function()
    if isToggleActive("Enable Jump") then
        ...
        if root and hum then
            -- Slider'dan TP miktarını al
            local tpBoost = getSlider("Enable Jump", "TP Boost") or 5
            
            -- Eğer TP Boost 0'dan büyükse ışınla
            if tpBoost > 0 then
                -- Karakteri anlık yukarı ışınla
                root.CFrame = root.CFrame * CFrame.new(0, tpBoost, 0)
                -- Fiziksel bir itme ver (bazı anti-cheat'leri atlatmak için)
...
addSlider("Speed", "Speed",15,30,20)
